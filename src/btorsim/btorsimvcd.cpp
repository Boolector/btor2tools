...
std::map<int64_t, std::string> BtorSimVCDWriter::read_info_file(const char* info_path)
{
  ...
  if (infofile.is_open()) {
    ...
    while (getline(infofile, line)) {
      ...
      msg (1, "Failed to parse line %d in info file: %s", lineno, line.c_str());
    }
    infofile.close();
  }
  return extra_bads;
...
void BtorSimVCDWriter::write_node_header (ModuleTreeNode* top)
{
  ...
  for (auto i : top->wire_names)
  {
    ...
    if (bv_identifiers.find(id) != bv_identifiers.end())
    {
      std::string type = (clocks.find(id) != clocks.end() && clocks[id] == EVENT) ? "event" : "wire";
      vcd_file << "$var " << type << " " << width << " " << bv_identifiers[i.first] << " " << symbol << " $end\n";
    }
    else
