#!/bin/sh
grep BTOR2_FORMAT_TAG_ src/btor2parser/btor2parser.h | \
sed -e 's,.*TAG_,,' -e 's/,.*$//g' | \
sort > /tmp/btor2parser-run-coverage-tags-in-header
grep 'PARSE (' src/btor2parser/btor2parser.c | \
sed -e 's,.*PARSE (,,' -e 's/,.*//g' | \
...
make clean && ./configure.sh -g -c && make && ./runtests.sh && gcov -o build src/btor2parser/btor2parser.c
